...
<html>
    <head>
        ...
        <script>
            ...
            function updateDrug() {
                demo.showNotification('top', 'right', 'Success Updated Drug', 'success');
            }
            function deleteDrug() {
                demo.showNotification('top', 'right', 'Success Deleted Drug','success');
            }
            function error() {
    ...
    <body
        <c:if test="${not empty param['add']}">  onload="addDrug();" </c:if>
        <c:if test="${not empty param['delete']}">  onload="deleteDrug();" </c:if>
        <c:if test="${not empty param['update']}">  onload="updateDrug();" </c:if>
            ...
            <div class="main-panel">
                ...
                <c:choose>
                    <c:when test="${ not empty param['uempid'] }">
                        <jsp:include page="pharmacy/updateDrug.jsp" flush="true" />
                    </c:when>
                    <c:otherwise>
                        <jsp:include page="pharmacy/addDrug.jsp" flush="true" />
                    </c:otherwise>
                </c:choose>
